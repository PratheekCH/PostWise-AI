    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6 divide-y divide-slate-100">
      {sortedPosts.map((post) => {
        ...
        return (
          <div
            key={post._id}
            className="py-4 first:pt-0 last:pb-0 flex flex-col md:flex-row md:items-start justify-between gap-4 group hover:bg-slate-50/70 px-3 rounded-xl transition-colors"
          >
              ...
              {post.hashtags && post.hashtags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {(Array.isArray(post.hashtags) ? post.hashtags : [post.hashtags]).map((tag, i) => (
                    <span key={i} className="text-[10px] text-indigo-500 font-medium">
                      {tag}
                    </span>
                  ))}
                </div>
